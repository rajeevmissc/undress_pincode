...
const tier = (price, days) => ({ available: true, price, transitDays: days, transitLabel: null });
// 1. DTDC full service: Surface + Air + COD
{
    ...
    const r = (0, rateResolver_1.resolveFromRecord)("110001", rec);
    ...
    assert((0, rateResolver_1.toPublicServiceability)(r).courier === "DTDC", "110001 public payload includes courier DTDC");
}
// 2. DTDC Surface-only (the 8,746-pincode real shape): no Air, no COD
{
    ...
    const r = (0, rateResolver_1.resolveFromRecord)("121014", rec);
...
// 3. DTDC Air-only (the 4-pincode real shape): no Surface
{
    ...
    const r = (0, rateResolver_1.resolveFromRecord)("744103", rec);
...
// 5. Delhivery-only, North East: Standard price 150 + COD 100, fixed transit label
{
    ...
    const r = (0, rateResolver_1.resolveFromRecord)("785101", rec);
...
// 6. Delhivery-only, North zone: Standard 120 + COD 50
{
    ...
    const r = (0, rateResolver_1.resolveFromRecord)("473554", rec);
...
// 7. Delhivery-only, prepaid but no COD
{
    ...
    const r = (0, rateResolver_1.resolveFromRecord)("999998", rec);
...
{
    const rNo = (0, rateResolver_1.resolveFromRecord)("792001", {
        serviceable: false,
        dtdc: { serviceable: false, surface: null, air: null, cod: { available: false, price: null, transitDays: null, transitLabel: null } },
        delhivery: { serviceable: false, standard: null, cod: { available: false, price: null, transitDays: null, transitLabel: null } },
    });
    ...
    assert((0, rateResolver_1.toPublicServiceability)(rNo).courier === "SPEEDPOST", "792001 public payload includes courier SPEEDPOST");
}
{
    const rNull = (0, rateResolver_1.resolveFromRecord)("123456", null);
    assert(rNull.serviceable === true && rNull.options.length === 1 && rNull.options[0].code === "SPEEDPOST", "missing record -> default Speed Post option");
